...
\begin{document}
% 5, part 1
\section{Hermitian Operators}\label{5}
...
\begin{align*}
\|Tx\|^4\ &\leq\ \left<Tx, x\right>\,\left< T y,y\right> \\
          &\leq\ \left<Ix, x\right>\,\left< I y,y\right>\\
          &  =\  \|x\|^2 \, \|y\|^2 \\
          &\leq\ \|x\|^2\,\|Tx\|^2
\end{align*}
and thereby
...
For $H=\R^2$
the elements of~$\mathscr H$ can be viewed as matrices
$\smash{\bigl(\begin{smallmatrix}a&b\\b&c\end{smallmatrix}\bigr)}$
(\ref{5.5}\ref{5.5-1}).
\begin{enumerate}[label=(\alph*)]
\item\label{5.11a}
Show: If 
$\smash{\bigl(\begin{smallmatrix}a&b\\b&c\end{smallmatrix}\bigr)}\geq 0$,
then
$\smash{\bigl(\begin{smallmatrix}a&-b\\-b&c\end{smallmatrix}\bigr)}\geq 0$.
...
I   := \begin{pmatrix}1&0\\0&1\end{pmatrix}\htam{, }\quad
A   := \begin{pmatrix}2&\sqrt{2}\\\sqrt{2}&2\end{pmatrix}\htam{.}
\end{equation*}
\end{enumerate}
\emph{Assume} that~$\{E_1,E_2\}$ has a supremum
$\smash{\bigl(\begin{smallmatrix}x&y\\y&z\end{smallmatrix}\bigr)}$
in~$\mathscr H$.
Show (using~\ref{5.11a}) that~$y=0$ and that
$\smash{\bigl(\begin{smallmatrix}x&y\\y&z\end{smallmatrix}\bigr)}$
can only be~$I$.
Show that~$A$ is an upper bound for~$\{E_1,E_2\}$
but $I\nleq A$.

Thus,
$\{E_1,E_2\}$ has no supremum.
...
$\mathscr H$ may not be closed under multiplication.
...
will return in Chapter~\ref{sec:dedcomp-ii}
\end{psec}
...
and $0\leq\left<Qx,x\right>\leq\left<x,x\right>$
for all~$x$ and~$y$;
in short:
\begin{equation*}
...
For a proof we may assume~$\|T \|=1$
and $\sqrt{T}=I - \lim q_n(I-T)$.
Let~$r_n$ be the polynomial
$t\mapsto q_n(t)-q_n(1-t)$;
then~$r_n$ has the value~$0$ at~$0$,
so~$r_n(T)$ lies in the linear hull of $\{T,T^2,T^3,\dotsc\}$.
But $\sqrt{T}=\lim r_n(T)$,
since
\begin{equation*}
(I-q_n(I-T)) - r_n(T) \ =\ (1-q_n(1))\,I
\end{equation*}
for each~$n$, and $q_n(1)\ra 1$.

The moral of this remark is that~$\sqrt{T}$
