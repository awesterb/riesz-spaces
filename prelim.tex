...
The \keyword{positive cone} in an ordered vector space~$E$ is
\begin{equation*}
...
\item %
\label{1.4-6}  
$ $ % otherwise "(6)" moves to the left for some unknown reason
\begin{equation*}
...
\varphi(0) &= 0\htam{, } &
\Var{[0,a]} \varphi &< \infty \quad(a\in\Lambda)
\htam{.}
\end{align*}
...
For $\varphi\in\Mod(\Lambda)$ we have
\begin{equation*}
|\varphi|(a)\ =\ \Var{[0,a]}{\varphi}\qquad(a\in\Lambda)\htam{.}
\end{equation*}
} % statement
\end{psec}
\begin{proof}
...
\statement{For $\mu \in \BA(\mathcal A)$, $\mu^+$ is given by
\begin{equation*}
\mu^+(A) 
...
\item
\label{1.14-2}
$\sigma(\mathcal A)$ is a Riesz space and is, 
in fact,
